...
export function RepositoryMetaInfo({ homepage, fork, created_at, pushed_at, language, topics, html_url }) {
  return (
    <RepositoryMetaInfoStyled>
      ...
      {fork === true && <RepositoryMetaInfoItem>Es Fork: Sí</RepositoryMetaInfoItem>}

      <RepositoryMetaInfoItem>Creado: {formatDate(created_at)}</RepositoryMetaInfoItem>
      <RepositoryMetaInfoItem>Último Push: {formatDate(pushed_at)}</RepositoryMetaInfoItem>

      {language && <RepositoryMetaInfoItem>Lenguaje más usado: {language}</RepositoryMetaInfoItem>}

      {topics.length > 0 && (
        <RepositoryMetaInfoItem>Tópicos: {topics.map((topic) => topic).join(', ')}</RepositoryMetaInfoItem>
      )}
